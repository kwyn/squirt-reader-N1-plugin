...
import SquirtStore from './../lib/squirt.store';
import { NylasStore } from 'nylas-exports';

import fs from 'fs';
const ipsumhtml = fs.readFileSync('resources/loremipsum.html', 'UTF-8');
let parser = {};

describe('SquirtStore', () => {
  // beforeEach(() => {
  //   parser = new SquirtParser();
  // });
  it('should be an instance of NylasStore', () => {
    let store = new SquirtStore();
    expect(store).to.be.an.instanceof(NylasStore);
  });
